    ...
    def run(self):
        ...
        with tqdm(total=self.bandit.T, postfix=postfix) as pbar:
            ...
            for t in range(self.bandit.T):
                ...
                if int(self.action) == int(self.bandit.best_actions_oracle[t]):
                    self.i += 1
                    self.score += self.bandit.best_rewards_oracle[t] 
                postfix['% optimal arm'] = '{:.2%}'.format(n_optimal_arm / self.iteration)
                postfix['ctr'] = '{:.4%}'.format(self.score / self.i)
                postfix['i'] = self.i
                postfix['score'] = self.score
